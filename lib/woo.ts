...
export async function findCustomerByEmail(email: string) {
  const url = `${WP_BASE}/wp-json/wc/v3/customers?email=${encodeURIComponent(email)}`;
...
export async function fetchProductCategories({ parent, slug, per_page = 100 }: { parent?: number; slug?: string; per_page?: number } = {}) {
